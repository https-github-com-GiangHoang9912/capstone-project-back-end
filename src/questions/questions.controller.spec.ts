...
describe('QuestionController', () => {
  ...
  it('get question by Exam', async () => {
    expect(await controller.getQuestionByExamId(req.res,1)).toEqual(
      req.res,
    );
    expect(mockQuestion.getQuestionsByExamId).toHaveBeenCalled();
  });
  it('delete question', async () => {
    expect(await controller.deleteQuestionById(req.res,1)).toEqual(
      req.res,
    );
    expect(mockQuestion.deleteQuestion).toHaveBeenCalled();
  });
  it('get question detail', async () => {
    expect(await controller.getQuestionDetail(req.res,1)).toEqual(
      req.res,
    );
  ...
  it('create question', async () => {
    ...
    expect(mockQuestion.createNewQuestion).toEqual({
      id: 1,
      questionBankId: 1,
      examId: 1,
     });
